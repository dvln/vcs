...
// comes in as a parameter to get local repo path and remote location
// info and Exists from the Existence intfc is also needed (sometimes)..
...
type Updater interface {
	// Describer gives access to VCS system details (Remote, LocalRepoPath, ..)
	Describer

	// Exists will determine if the repo exists (remotely or in localPath)
...
//	remoteName (string): "" or remote repo "name" (eg: "origin" is default for git)
...
func NewUpdater(remote, remoteName, localPath string, mirror bool, rebase RebaseVal, refs map[string]RefOp, vcsType ...Type) (Updater, error) {
	...
	switch vtype {
	case Git:
		return NewGitUpdater(remote, remoteName, localPath, mirror, rebase, refs)
	case Svn:
		return NewSvnUpdater(remote, remoteName, localPath, mirror, rebase, refs)
	case Hg:
		return NewHgUpdater(remote, remoteName, localPath, mirror, rebase, refs)
	case Bzr:
		return NewBzrUpdater(remote, remoteName, localPath, mirror, rebase, refs)
	}
